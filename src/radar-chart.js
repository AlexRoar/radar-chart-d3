var RadarChart = {
    ...
    chart: function () {
        ...
        function clamp(newV, minV, maxV) {
            return newV > maxV ? maxV : (newV < minV ? minV : newV);
        }
            ...
            selection.each(function (data) {
                ...
                if (cfg.circles && cfg.radius) {
                    ...
                    function move(dobj, i) {
                        ...
                        const width = cfg['w'] / 2
                        const height = cfg['h'] / 2
                        const maxValue = cfg['maxValue']

                        const oldX = parseFloat(dragTarget.attr("cx")) - width;
                        const oldY = height - parseFloat(dragTarget.attr("cy"));
                        let newY = 0, newX = 0;
                        const len = data[0].axes.length

                        let maxY = height, maxX = width
                        let minY = 0.0001, minX = 0.0001
                        ...
                        if (i === 0 || i === len / 2) {
                            newY = clamp(oldY - d3.event.dy, minY, maxY);
                        } else {
                            const slope = oldY / oldX;
                            newX = clamp(oldX + d3.event.dx, minX, maxX)
                            newY = newX * slope
                        ...
                        let newValue = Math.sqrt(newX * newX + newY * newY) /
                            (cfg.factor * width) * maxValue;
                        ...
                        data[0].axes[i].value = newValue
                        const oldPoints = polygon.attr('points').split(' ')
